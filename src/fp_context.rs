...
impl PySDFContext {
    ...
    pub fn to_vm(&mut self, expr: &PyExpr) -> PyResult<String> {
        ...
        // Phase 1: Collect all nodes in topological order (post-order DFS = children before parents)
        fn collect_nodes(
            node: fidget::context::Node,
            ctx: &Context,
            visited: &mut HashSet<*const fidget::context::Op>,
            ordered: &mut Vec<fidget::context::Node>,
        ) {
            ...
            match op {
                fidget::context::Op::Binary(_, a, b) => {
                    collect_nodes(*a, ctx, visited, ordered);
                    collect_nodes(*b, ctx, visited, ordered);
                }
        ...
        let mut visited: HashSet<*const fidget::context::Op> = HashSet::new();
        let mut ordered: Vec<fidget::context::Node> = Vec::new();
        collect_nodes(node, &self.ctx, &mut visited, &mut ordered);

        // Phase 2: Stable-sort so vars come first, then consts, then ops.
        // Stable sort preserves topological order within each group.
        ordered.sort_by_key(|&n| match self.ctx.get_op(n).unwrap() {
            fidget::context::Op::Input(_) => 0u8,
            fidget::context::Op::Const(_) => 1,
            _ => 2,
        });

        // Phase 3: Assign sequential IDs and write
        let mut output = Vec::new();
        writeln!(output, "# Fidget VM format export").unwrap();
        writeln!(output, "# Generated by fidgetpy").unwrap();

        let mut node_to_id: HashMap<*const fidget::context::Op, String> = HashMap::new();
        let mut next_id: u32 = 0;

        for &n in &ordered {
            let op = self.ctx.get_op(n).unwrap();
            let ptr = op as *const _;
            let id = format!("_{:x}", next_id);
            next_id += 1;
            node_to_id.insert(ptr, id.clone());

            match op {
                fidget::context::Op::Input(v) => match v {
                    Var::X => writeln!(output, "{} var-x", id).unwrap(),
                    Var::Y => writeln!(output, "{} var-y", id).unwrap(),
                    Var::Z => writeln!(output, "{} var-z", id).unwrap(),
                    _ => {
                        if let Some(name) = expr.var_names.get(v) {
                ...
                fidget::context::Op::Unary(opcode, a) => {
                    let a_ptr = self.ctx.get_op(*a).unwrap() as *const _;
                    let a_id = node_to_id.get(&a_ptr).unwrap();
                ...
                fidget::context::Op::Binary(opcode, a, b) => {
                    let a_ptr = self.ctx.get_op(*a).unwrap() as *const _;
                    let b_ptr = self.ctx.get_op(*b).unwrap() as *const _;
                    let a_id = node_to_id.get(&a_ptr).unwrap();
                    let b_id = node_to_id.get(&b_ptr).unwrap();
                    let op_name = match opcode {
