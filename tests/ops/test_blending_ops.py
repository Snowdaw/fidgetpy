...
def test_smooth_min():
    ...
    result = fp.eval(smooth_min_sdf, points)
...
def test_exponential_smooth_min():
    ...
    result = fp.eval(exp_smooth_min_sdf, points)
    assert len(result) == len(points)
...
def test_power_smooth_min():
    ...
    result = fp.eval(power_smooth_min_sdf, points)
    assert len(result) == len(points)
...
def test_smooth_max():
    ...
    result = fp.eval(smooth_max_sdf, points)
    assert len(result) == len(points)
    ...
    zero_smooth = fp.eval(fpo.smooth_max(sphere, cube, 0.000001), points)
...
def test_exponential_smooth_max():
    ...
    result = fp.eval(exp_smooth_max_sdf, points)
    assert len(result) == len(points)
...
def test_power_smooth_max():
    ...
    result = fp.eval(power_smooth_max_sdf, points)
    assert len(result) == len(points)
...
def test_soft_clamp():
    ...
    result = fp.eval(soft_clamp_sdf, points)
    assert len(result) == len(points)
    
    # Basic validation: with k=0, should match hard clamp
    sphere_values = fp.eval(sphere, points)
    expected_clamped = np.clip(sphere_values, -0.5, 0.5)
    zero_smooth = fp.eval(fpo.soft_clamp(sphere, -0.5, 0.5, 0.000001), points)
